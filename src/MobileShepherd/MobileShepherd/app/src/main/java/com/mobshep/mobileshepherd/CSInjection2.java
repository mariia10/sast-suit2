 ...
 * @author Sean Duggan
 */
public class CSInjection2 extends MainActivity
      ...
      case (R.id.bLogin):
        ...
        try {
          if (login(Name, Pass) == true) {
            ...
            Toast toast = Toast.makeText(CSInjection2.this, "Logged in!", Toast.LENGTH_LONG);
            toast.show();
          }
        } catch (IOException e1) {
          Toast toast = Toast.makeText(CSInjection2.this, "An error occurred!", Toast.LENGTH_LONG);
          toast.show();
        }
        ...
        try {
          if (login(Name, Pass) == false) {
            Toast toast =
                Toast.makeText(
                    CSInjection2.this, "Invalid Credentials, " + Name, Toast.LENGTH_LONG);
            toast.show();
          }
        } catch (IOException e) {
        ...
        if (Name.contentEquals("") || Pass.contentEquals("")) {
          Toast toast2 =
              Toast.makeText(CSInjection2.this, "Empty Fields Detected.", Toast.LENGTH_SHORT);
          toast2.show();
        }
    }
  }
  ...
  private boolean login(String username, String password) throws IOException {
    try {
      try {
        ...
        String query =
            ("SELECT * FROM Players WHERE memName='"
                + username
                + " AND memPass = '"
                + password
                + ";");
        ...
        // TODO Auto-generated catch block
        Toast error = Toast.makeText(CSInjection2.this, "An error occurred.", Toast.LENGTH_LONG);
        error.show();
        key.getText().clear();
      ...
      Toast error =
          Toast.makeText(CSInjection2.this, "An database error occurred.", Toast.LENGTH_LONG);
      error.show();
    }
  ...
  public void populateTable(Context context, String dbPassword) {
    try {
      ...
      try {
        ...
        db.execSQL(
            "CREATE TABLE Players(memID INTEGER PRIMARY KEY AUTOINCREMENT, memName TEXT, memAge"
        ...
        Toast error =
            Toast.makeText(
                CSInjection2.this, "An error occurred, table not initialized.", Toast.LENGTH_LONG);
        error.show();
      }
  ...
  public void generateKey(Context context, String password) {
    try {
      try {
        ...
        Toast error =
            Toast.makeText(
                CSInjection2.this, "An error occurred, key was not generated", Toast.LENGTH_LONG);
        error.show();
      }
