...
public class poorAuth2_Main extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (!f.exists()) {
      ...
      try {
        copyCache(
            getBaseContext().getAssets().open("usedAuthCodes"),
    ...
    ArrayAdapter<String> adapter =
        new ArrayAdapter<String>(this, android.R.layout.simple_dropdown_item_1line, authCodes);
  ...
  private void showError() {
    Toast loginFailed =
        Toast.makeText(poorAuth2_Main.this, "Invalid Credentials!", Toast.LENGTH_LONG);
    loginFailed.show();
  }
  ...
  public void copyCache(InputStream iStream, OutputStream oStream) throws IOException {
