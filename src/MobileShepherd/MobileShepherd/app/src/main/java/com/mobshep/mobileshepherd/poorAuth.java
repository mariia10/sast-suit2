  ...
  public void onClick(View arg0) {
    switch (arg0.getId()) {
      ...
      case (R.id.bLogin):
        ...
        Log.d(
            TAG, "temp pass = " + poorAuth_Reset.tempPassVar + "passwordReset = " + passwordReset);

        if (passwordReset == true) {
          if (CheckName.equals(usernameVar)) {

            if (CheckPass.equals(tempPass)) {
              Toast loggedIn = Toast.makeText(poorAuth.this, "Logged in!", Toast.LENGTH_LONG);
              loggedIn.show();

              Intent loggedInIntent = new Intent("com.mobshep.mobileshepherd.PoorAuth_Main");
              startActivity(loggedInIntent);
            }

          } else {
            Toast locked = Toast.makeText(poorAuth.this, "Invalid Password!", Toast.LENGTH_SHORT);
            locked.show();
          ...
          Toast locked =
              Toast.makeText(poorAuth.this, "You're account has been locked!", Toast.LENGTH_SHORT);
          locked.show();
          break;
        }

        if (CheckName.contentEquals("") || CheckPass.contentEquals("")) {
          Toast empty = Toast.makeText(poorAuth.this, "Empty Fields Detected.", Toast.LENGTH_SHORT);
          empty.show();
        }
        ...
        if (CheckName.equals("Jack") == false
          ...
          Log.d(
              TAG,
              "temp pass = " + poorAuth_Reset.tempPassVar + "passwordReset = " + passwordReset);
          break;
        }
        break;
    }
