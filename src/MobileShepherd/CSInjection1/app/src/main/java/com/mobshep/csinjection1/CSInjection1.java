...
public class CSInjection1 extends Activity implements OnClickListener {
      ...
      case (R.id.bLogin):
        ...
        try {
          if (login(sanitizeName, sanitizePass) == false) {
            Toast invalid =
                Toast.makeText(
                    CSInjection1.this, "Invalid Credentials, " + sanitizeName, Toast.LENGTH_LONG);
            invalid.show();
          }
        } catch (IOException e) {
        ...
        if (sanitizeName.contentEquals("") || sanitizePass.contentEquals("")) {
          Toast blank =
              Toast.makeText(CSInjection1.this, "Empty Fields Detected.", Toast.LENGTH_SHORT);
          blank.show();
        }
    }
  }
  ...
  private boolean login(String username, String password) throws IOException {
    try {
      ...
      Toast error =
          Toast.makeText(CSInjection1.this, "An database error occurred.", Toast.LENGTH_LONG);
      error.show();
    }
