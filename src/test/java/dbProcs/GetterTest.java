...
/**
 * Class is targeted to test all of the methods found in the src/dbprocs/Getter.java class, but does
 * include some coverage of other classes, such as Setter.java and Database.java
 *
 * @author mark
 */
public class GetterTest {
  ...
  /**
   * Searches for class based on class name. If nothing is found, the class is created and the new
   * class Id is returned
   *
  ...
  public static String findCreateClassId(String className, String applicationRoot)
    ...
    if (classId.isEmpty()) {
      ...
      if (Setter.classCreate(applicationRoot, className, "2015")) {
        ...
        TestProperties.failAndPrint("Could not create create class " + className);
      }
    }
    return classId;
  }
  ...
  /**
   * This method will sign in as an admin, or create the admin and sign in as them. If this fails it
   * will throw an Exception. This function will pass if correct user credentials are passed as well
   *
  ...
  public static boolean verifyTestAdmin(String applicationRoot, String userName, String password)
    ...
    if (user == null || user[0].isEmpty()) {
      ...
      Setter.userCreate(
          applicationRoot, null, userName, userName, "admin", userName + "@test.com", false);
    ...
    if (user != null && !user[0].isEmpty()) {
      ...
      TestProperties.failAndPrint(
          "Could not verify that user " + userName + " could authenticate at all.");
    }
  ...
  /**
   * This method will sign in as a User, or create the user and sign in as them. If this fails it
  ...
  public static boolean verifyTestUser(String applicationRoot, String userName, String password)
    ...
    if (user == null || user[0].isEmpty()) {
      log.debug("User not found in DB. Adding user to DB and retesting before giving up");
      Setter.userCreate(
          applicationRoot, null, userName, password, "player", userName + "@example.com", false);
  ...
  /**
   * This method will sign in as a User, or create the user and sign in as them. If this fails it
   * will throw an Exception. They will be added to the submitted class
   *
   * @param applicationRoot Context of running application
   * @param userName The user name of the user you want to create or sign in as
   * @param password The password of the user you want to create or sign in as
   ...
   */
  public static boolean verifyTestUser(
      String applicationRoot, String userName, String password, String theClass)
      throws SQLException {
    boolean result = false;
    ...
    if (user == null || user[0].isEmpty()) {
      log.debug("User not found in DB. Adding user to DB and Retesting before giving up");
      Setter.userCreate(
          applicationRoot, theClass, userName, userName, "player", userName + "@test.com", false);
      user = Getter.authUser(applicationRoot, userName, userName);
    }
    if (user != null && !user[0].isEmpty()) {
      ...
      TestProperties.failAndPrint(
          "Could not verify that " + userName + " could authenticate at all");
    }
  ...
  public void testAuthUserCorrectUpperCaseCredentials() {
    ...
    try {
      ...
      if (user == null || user[0].isEmpty()) {
        ...
        Setter.userCreate(
            applicationRoot, null, userName, password, "player", userName + "@test.com", false);
          ...
          if (!Getter.getModuleAddress(applicationRoot, dataStorageLessonId, userId).isEmpty()) {
            ...
            String markLevelCompleteTest =
                Setter.updatePlayerResult(
                    applicationRoot, dataStorageLessonId, userId, "Feedback is Disabled", 1, 1, 1);
            if (markLevelCompleteTest != null) {
              String checkPlayerResultTest =
                  Getter.checkPlayerResult(applicationRoot, dataStorageLessonId, userId);
  ...
  public void testCheckPlayerResultWhenModuleNotOpened() {
    ...
    String unOpenedModuleId =
        new String("0dbea4cb5811fff0527184f99bd5034ca9286f11"); // Insecure Direct Object
    ...
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (test != null) {
          ...
          fail(
              "Function says User has opened module they should not have opened by"
                  + " default"); // User Should
          // not have
          // completed
          // this
          ...
          if (!Getter.getModuleAddress(
            ...
            if (test == null) {
              fail(
                  "Function says "
                      + userName
                      + " has not opened module"); // User Should have opened and
              // not completed CSRF Three
            } else {
              return; // Pass
            }
          } else {
      ...
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (!modules.isEmpty()) // Some Modules were included in response
          {
            ...
            int numberofChallengesReturned =
                (modules.length() - modules.replace("class='lesson'", "").length())
                    / "class='lesson'".length();
            if (numberofChallengesReturned > totalNumberOfModulesInShepherd) {
              ...
              log.debug(
                  "Too Few Challenges Returned to pass: "
                      + numberofChallengesReturned
                      + " returned. Expected at least:"
                      + totalNumberOfModulesInShepherd);
        ...
        if (Setter.closeAllModules(applicationRoot)) {
          ...
          if (!modules.isEmpty()) // Some Modules were included in response
          {
            ...
            if (!(numberofChallengesReturned > 0)) {
              ...
              log.debug(
                  "Too Many Challenges Returned to pass: "
                      + numberofChallengesReturned
                      + " returned");
    ...
    try {
      ...
      if (classId.isEmpty()) {
        ...
        if (classInfo[0].equalsIgnoreCase("NewClassForGetInfo2")
            && classInfo[1].equalsIgnoreCase("2015")) {
    ...
    try {
      ...
      if (verifyTestUser(applicationRoot, userName, userName, classId)) {
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          // Simulate user Opening Level
          if (!Getter.getModuleAddress(
                  applicationRoot, moduleId, Getter.getUserIdFromName(applicationRoot, userName))
              .isEmpty()) {
            String csrfFourm =
                Getter.getCsrfForumWithIframe(applicationRoot, classId, moduleId, bundle);
        ...
        if (verifyTestUser(applicationRoot, userName, userName, classId)) {
          ...
          if (Setter.openAllModules(applicationRoot, false)
            ...
            if (!Getter.getModuleAddress(
                ...
                .isEmpty()) {
              String csrfFourm =
                  Getter.getCsrfForumWithImg(applicationRoot, classId, moduleId, bundle);
              if (csrfFourm.indexOf(userName) > -1) {
                log.debug("PASS: User was found in the fourm");
                return;
              } else {
      ...
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (!Getter.getModuleAddress(applicationRoot, dataStorageLessonId, userId).isEmpty()) {
            ...
            String markLevelCompleteTest =
                Setter.updatePlayerResult(
                    applicationRoot, dataStorageLessonId, userId, feedbackSearchCode, 1, 1, 1);
            if (markLevelCompleteTest != null) {
              String checkPlayerResultTest =
                  Getter.checkPlayerResult(applicationRoot, dataStorageLessonId, userId);
              log.debug("checkPlayerResultTest" + checkPlayerResultTest);
              if (checkPlayerResultTest == null) {
                log.debug(
                    "Checking to see if the feedback is included in the getFeeback response for the"
                        + " module");
                ...
                if (feedback.indexOf(feedbackSearchCode) > -1) {
                  ...
                  log.fatal(
                      "User's Feedback '" + feedbackSearchCode + "' was not found in: " + feedback);
  ...
  public void testGetIncrementalModulesWithModulesClosed() {
    ...
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.closeAllModules(applicationRoot)) {
          String incrementalModules =
              Getter.getIncrementalModules(applicationRoot, userId, lang, "testingCSRFtoken");
          if (incrementalModules.indexOf("You've Finished!")
              > -1) // IF no modules are open, this is the
          // expected leading string
          {
  ...
  public void testGetIncrementalModulesWithNoneComplete() {
    ...
    String lowestRankModuleId =
        "0dbea4cb5811fff0527184f99bd5034ca9286f11"; // This should be changed if an easier
    // module is made
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        String userId = Getter.getUserIdFromName(applicationRoot, userName);
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (incrementalModules.indexOf("Completed")
              == -1) // User should not have completed any modules.
          // The Completed Button should not be present
          {
            if (incrementalModules.indexOf(lowestRankModuleId)
                > -1) // The only module Id to be returned
            // should be this one as it is the
            // first presented (Lowest
            // Incremental Rank)
            {
              if (incrementalModules.indexOf("Get Next Challenge")
                  > -1) // This is the English string
              // that should be included with
              // the lang submitted in this
              // unit test
              {
                log.debug(
                    "PASS: Incremental Menu Appears to have Rendered correctly with the"
                        + " Preconditions of this test");
                return;
              } else {
              ...
              fail(
                  "The Module Id Returned was not the Known First Level. Ie not: "
                      + lowestRankModuleId);
            }
          } else {
  ...
  public void testGetIncrementalModulesWithOneModuleComplete() {
    ...
    String secondLowestRankModuleId =
        "b9d82aa7b46ddaddb6acfe470452a8362136a31e"; // This should be changed if an
    // easier module is made or is
    // orded before this
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        String userId = Getter.getUserIdFromName(applicationRoot, userName);
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (!Getter.getModuleAddress(applicationRoot, lowestRankModuleId, userId).isEmpty()) {
            ...
            if (markLevelCompleteTest != null) {
              String checkPlayerResultTest =
                  Getter.checkPlayerResult(applicationRoot, lowestRankModuleId, userId);
              log.debug("checkPlayerResultTest" + checkPlayerResultTest);
              if (checkPlayerResultTest == null) {
                String incrementalModules =
                    Getter.getIncrementalModules(applicationRoot, userId, lang, "testingCSRFtoken");
                if (incrementalModules.indexOf("Completed") > -1) // User should have completed one
                // module. The Completed Button
                // should be present
                {
                  if (incrementalModules.indexOf(lowestRankModuleId) > -1) // The only completed
                  // module Id to be
                  // returned should be
                  // this one
                  {
                    if (incrementalModules.indexOf(secondLowestRankModuleId) > -1) {
                      if (incrementalModules.indexOf("Get Next Challenge") > -1) // This is the
                      // English
                      // string that
                      // should be
                      // included with
                      ...
                      fail(
                          "The Module Id Returned to be Completed Next was not the Known 2nd Level."
                              + " Ie not: "
                              + secondLowestRankModuleId);
                    }
                  } else {
                    fail(
                        "The Module Id Returned was not the Known First Level. Ie not: "
  ...
  public void testGetIncrementalModulesWithoutScriptWithModulesClosed() {
    ...
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.closeAllModules(applicationRoot)) {
          String incrementalModules =
              Getter.getIncrementalModulesWithoutScript(
          ...
          // expected leading string
          {
            if (!incrementalModules.endsWith(";</script>")) {
              log.debug(
                  "PASS: Incremental Menu Appears to have Rendered correctly with the Preconditions"
                      + " of this test without ending in the button script");
              return;
            } else {
              log.debug("incrementalModules returned: " + incrementalModules);
            ...
            // Incremental Rank)
            {
              ...
              // unit test
              {
                if (!incrementalModules.endsWith(";</script>")) {
                  log.debug(
                      "PASS: Incremental Menu Appears to have Rendered correctly with the"
                          + " Preconditions of this test without ending in the button script");
                  return;
                } else {
                  log.debug("incrementalModules returned: " + incrementalModules);
                  fail("Function Ended in Unexpected Script");
                    ...
                    if (incrementalModules.indexOf(secondLowestRankModuleId) > -1) {
                      ...
                      // test
                      {
                        if (!incrementalModules.endsWith(";</script>")) {
                          log.debug(
                              "PASS: Incremental Menu Appears to have Rendered correctly with the"
                                  + " Preconditions of this test without ending in the button"
                                  + " script");
                          return;
                        } else {
                          log.debug("incrementalModules returned: " + incrementalModules);
  ...
  /**
   * Tests to ensure that user can only see their data in the scoreboard, and cannot see the data
   * from users in other classes in the scoreboard
   */
  @Test
  public void testGetJsonScoreClassSpecific() {
    ...
    String insecureDirectObjectRefLesson =
        "0dbea4cb5811fff0527184f99bd5034ca9286f11"; // Direct Object Reference
    // Module
    try {
      try {
        classId = findCreateClassId(className);
  ...
  /**
   * Test to ensure users that have not scored any points, or are on negative points are not shown
   * in the scoreboard
   */
  @Test
  public void testGetJsonScoreTotalNoneOrNegPoints() {
    ...
    try {
      ...
      if (verifyTestUser(applicationRoot, userName, userName, classId)
          && verifyTestUser(applicationRoot, otherUserName, otherUserName, classId)) {
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (Setter.updateUserPoints(applicationRoot, otherUserId, -5)) {
            ...
            if (scoreboardData.isEmpty()) {
              log.debug(
                  "PASS: The Scoreboard response was empty. Therefore the users are not valid to be"
                      + " returned");
              return; // PASS
            }
            ...
            log.debug(
                "PASS: Did not ether user's in the response, therefore they were not included");
            return; // PASS
          } else {
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (markLevelCompleteTest != null) {
            ...
            for (int i = 0; i < scoreboardJson.length(); i++) {
              ...
              if (scoreRowJson.get("username").toString().compareTo(otherUserName)
                  == 0) // Therefore not
              // encoded for
              // HTML
              {
            ...
            log.debug(
                "PASS: Did not find HTML Strings in Scoreboard Response. Therefore they are"
                    + " encoded");
            return; // PASS
          } else {
            fail("Failed to Mark Direct Object Level as Complete for 2nd User");
    ...
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.openAllModules(applicationRoot, false)
            && Setter.openAllModules(applicationRoot, true)) {
          // Simulate user Opening Level
          if (!Getter.getModuleAddress(applicationRoot, inscureDirectObjectLesson, userId)
              .isEmpty()) {
            String markLevelCompleteTest =
                Setter.updatePlayerResult(
                    applicationRoot,
                    inscureDirectObjectLesson,
                    userId,
                    "Feedback is Disabled",
                    1,
                    1,
            ...
            if (markLevelCompleteTest != null) {
              ...
              if (lessonsMenu.indexOf("class='lesson'")
                  > -1) // Menu Should include this at least once
              {
                if (lessonsMenu.indexOf(inscureDirectObjectLesson)
                    > -1) // This module should be in the
                // response
                {
                  if (lessonsMenu.indexOf(
                          "<img src='css/images/completed.png'/><a class='lesson' id='"
                              + inscureDirectObjectLesson)
                      > -1) // This module should be returned
                  // as completed
                  {
                    if (lessonsMenu.indexOf(
                            "<img src='css/images/uncompleted.png'/><a class='lesson' id='"
                                + poorDataValidationLesson)
                        > -1) {
                      if (lessonsMenu.indexOf("Insecure Direct Object References") > -1) // English
                      // string
                      // should
                      // exist
                      // in
                      ...
                      // locale
                      {
                        ...
                        log.fatal(
                            "Could not find i18n English String in lessons Menu: " + lessonsMenu);
                        fail("Could not Detect i18n Locale Strings In Lessons Menu");
                      }
                    } else {
                      log.fatal(
                          "Could not detect Uncompleted Icon beside Poor Data Validation Lesson: "
                              + lessonsMenu);
                      fail("Uncompleted Module did not have Uncopmleted Symbol");
                    }
                  } else {
                    log.fatal(
                        "Could not detect completed Icon beside Insecure Direct Object Reference"
                            + " Lesson");
                    log.error(
                        "Could not find : <img src='css/images/completed.png'/><a class='lesson'"
                            + " id='"
                            + inscureDirectObjectLesson
                            + " in "
                            + lessonsMenu);
                  ...
                  log.fatal(
                      "Could not find Insecure Direct Object References ModuleID in: "
                          + lessonsMenu);
    ...
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.closeAllModules(applicationRoot)) {
          String lessonsMenu = Getter.getLessons(applicationRoot, userId, locale);
          if (lessonsMenu.indexOf("class='lesson'")
              == -1) // Menu Should not include this when modules closed
          {
            if (lessonsMenu.indexOf("No lessons found")
                > -1) // English string should exist in output based
            // on the submitted locale
            {
              log.debug(
                  "PASS: GetLessons Menu Appears to have Rendered correctly with the Preconditions"
                      + " of this test");
              return;
            } else {
  ...
  public void testGetModuleCategory() {
    ...
    if (Getter.getModuleCategory(applicationRoot, insecureCryptoLesson)
            .compareTo("Insecure Cryptographic Storage")
        != 0) {
  ...
  public void testGetModuleHash() {
    ...
    if (Getter.getModuleHash(applicationRoot, insecureCryptoLesson)
            .compareTo("if38ebb58ea2d245fa792709370c00ca655fded295c90ef36f3a6c5146c29ef2")
        != 0) {
  ...
  public void testGetModuleIdFromHash() {
    String insecureCryptoLesson = new String("201ae6f8c55ba3f3b5881806387fbf34b15c30c2");
    if (Getter.getModuleIdFromHash(
                applicationRoot, Getter.getModuleHash(applicationRoot, insecureCryptoLesson))
            .compareTo(insecureCryptoLesson)
        != 0) {
  ...
  public void testGetModuleResult() {
    String insecureCryptoLesson = new String("201ae6f8c55ba3f3b5881806387fbf34b15c30c2");
    String knownStoredResult =
        new String("base64isNotEncryptionBase64isEncodingBase64HidesNothingFromYou");
  ...
  public void testGetModuleResultFromHash() {
    String insecureCryptoLessonHash =
        new String("if38ebb58ea2d245fa792709370c00ca655fded295c90ef36f3a6c5146c29ef2");
    String knownStoredResult =
        new String("base64isNotEncryptionBase64isEncodingBase64HidesNothingFromYou");
    String methodReturnResult =
        Getter.getModuleResultFromHash(applicationRoot, insecureCryptoLessonHash);
    if (knownStoredResult.compareTo(methodReturnResult) != 0) {
      log.fatal(
          "Known Result ("
              + knownStoredResult
  ...
  public void testGetModulesInOptionTagsCTF() {
    ...
    if (modules.indexOf(lowestRankLevel) == -1) {
      ...
      log.fatal(
          "Wrong Module Listed First. Should be module with lowest incremental Rank: " + modules);
  ...
  public void testGetModuleSolution() {
    String insecureDirectObjectReferenceLesson =
        new String("0dbea4cb5811fff0527184f99bd5034ca9286f11");
    String[] moduleSolution =
        Getter.getModuleSolution(applicationRoot, insecureDirectObjectReferenceLesson, locale);
    if (moduleSolution == null) {
      fail("Could not retrieve module solution");
    } else if (moduleSolution[1].indexOf("Stop the request") == -1) {
      log.fatal(
          "Could not find 'Stop the request' in the following solution: " + moduleSolution[1]);
  ...
  public void testGetModuleStatusMenu() {
    ...
    if (Setter.openAllModules(applicationRoot, false)
      ...
      } else if (moduleStatusMenu.indexOf("id='toOpen'></select></td>")
          < 0) // Should be empty as all modules
      // should be open
      {
        log.fatal(
            "Modules are in the 'toOpen' list when all modules should already be open: "
                + moduleStatusMenu);
        fail("Modules are in the 'toOpen' list when all modules should already be open");
      } else {
        // Make Sub String for the toClose List
        int endOfToCloseMenu =
            moduleStatusMenu
                    .substring(moduleStatusMenu.indexOf("id='toClose'>"))
                    .indexOf("</select>")
                + (moduleStatusMenu.length()
                    - moduleStatusMenu
                        .substring(moduleStatusMenu.indexOf("id='toClose'>"))
                        .length());
        String toCloseList =
            moduleStatusMenu.substring(moduleStatusMenu.indexOf("id='toClose'>"), endOfToCloseMenu);
  ...
  public void testGetModuleStatusMenuWhenClosed() {
    ...
    if (Setter.closeAllModules(applicationRoot)) {
      ...
      if (moduleStatusMenu.indexOf("<tr><th>To Open</th><th>To Close</th></tr><tr>") == -1) {
        ...
        log.fatal("No Close Menu Detected in Output: " + moduleStatusMenu);
        fail("No Close Meny Detected in Output");
      } else if (moduleStatusMenu.indexOf("id='toClose'></select></td>")
          < 0) // Should be empty as all modules
      // should be closed
      {
        log.fatal(
            "Modules are in the 'toClose' list when all modules should already be closed: "
                + moduleStatusMenu);
        ...
        int endOfToOpenMenu =
            moduleStatusMenu
                    .substring(moduleStatusMenu.indexOf("id='toOpen'>"))
                    .indexOf("</select>")
                + (moduleStatusMenu.length()
                    - moduleStatusMenu
                        .substring(moduleStatusMenu.indexOf("id='toOpen'>"))
                        .length());
        String toOpenList =
            moduleStatusMenu.substring(moduleStatusMenu.indexOf("id='toOpen'>"), endOfToOpenMenu);
    ...
    if (Setter.openOnlyMobileCategories(applicationRoot)) {
      ...
      if (moduleStatusMenu.indexOf("<tr><th>To Open</th><th>To Close</th></tr><tr>") == -1) {
        ...
        fail("No Close Meny Detected in Output");
          ...
          > 0) // Should not be empty as Web Levels
      // should be closed
      {
        log.fatal(
            "Modules are in the 'toClose' list when web modules should already be closed: "
                + moduleStatusMenu);
        fail("Modules are in the 'toClose' list when web modules should already be closed");
      } else if (moduleStatusMenu.indexOf("id='toOpen'></select></td>")
          > 0) // Should not be empty as Mobile
      // Levels should be open
      {
        log.fatal(
            "Modules are in the 'toOpen' list when mobile modules should already be closed: "
                + moduleStatusMenu);
        ...
        if (toCloseList.indexOf(dataStorageLessonId) == -1) {
          ...
          // Verify the correct number of modules are open/closed (At this point the Menu
          // is fine. This is really now testing the mobile/web setter filters)
          int numberOfMobileLevelsOpen =
              (toCloseList.length() - toCloseList.replace("<option", "").length())
                  / "<option".length();
          log.debug(numberOfMobileLevelsOpen + " mobile levels detected");
          int numberOfWebLevelsClosed =
              (toOpenList.length() - toOpenList.replace("<option", "").length())
                  / "<option".length();
          ...
          try {
            ...
            PreparedStatement prepStatement =
                conn.prepareStatement(
                    "SELECT COUNT(*) FROM modules WHERE "
            ...
            prepStatement =
                conn.prepareStatement(
                    "SELECT COUNT(*) FROM modules WHERE "
                        + Setter.webModuleCategoryHardcodedWhereClause);
          ...
          if (mobileDbModuleCount != numberOfMobileLevelsOpen) {
            fail(
                "There are "
                    + numberOfMobileLevelsOpen
                    + " mobile levels in open list, but there are "
                    + mobileDbModuleCount
                    + " in the DB");
            // This may mean that the where clause in the setter method is not correct and
            // needs to be updated
          } else if (webDbModuleCount != numberOfWebLevelsClosed) {
            fail(
                "There are "
                    + numberOfWebLevelsClosed
                    + " web levels in open list, but there are "
                    + webDbModuleCount
                    + " in the DB");
            // This may mean that the where clause in the setter method is not correct and
            // needs to be updated
          } else if ((mobileDbModuleCount + webDbModuleCount)
              != (numberOfMobileLevelsOpen + numberOfWebLevelsClosed)) {
  ...
  public void testGetOpenCloseCategoryMenu() {
    String moduleCategory =
        "Injection"; // This will need to be updated to a locale key when this method is
    ...
    try {
      ...
      if (verifyTestUser(applicationRoot, userName, userName, classId)
        ...
        if (Setter.openAllModules(applicationRoot, false)
            && Setter.openAllModules(applicationRoot, true)) {
          // Simulate user Opening Level
          if (Getter.getModuleAddress(applicationRoot, insecureDirectObjectRefLesson, userId)
              .isEmpty()) {
            fail("Could not Simulate Opening Level for User 1");
          } else if (Getter.getModuleAddress(
                  applicationRoot, insecureDirectObjectRefLesson, otherUserId)
              .isEmpty()) {
            fail("Could not Simulate Opening Level for User 1");
          } else {
            String markLevelCompleteTest =
                ...
                String userRowStart =
                    new String(
                        userName
                            ...
                            + " ");
                int startOfProgressWidth =
                    classProgress.indexOf(userRowStart) + userRowStart.length();
                String firstCharacterOfLength =
                    classProgress.substring(startOfProgressWidth, startOfProgressWidth + 1);
                int lengthOfProgress =
                    Integer.parseInt(firstCharacterOfLength); // We dont care what
                // number it is. As
                // Long as it's not
                // 0
              ...
              if (jsonProgressString.indexOf(otherClassName) > 0) {
                ...
                for (int i = 0; i < jsonProgress.length(); i++) {
                  ...
                  if (userProgress.get("userName").toString().compareTo(userName) == 0) {
                    ...
                    if (progressBar <= 0) {
                      fail(
                          "User has no progress according to response when they have completed a"
                              + " level");
                    }
                  } else if (userProgress.get("userName").toString().compareTo(anotherUserName)
                      == 0) {
                    int progressBar = Integer.parseInt(userProgress.get("progressBar").toString());
                    if (progressBar != 0) {
          ...
          if (!Getter.getModuleAddress(applicationRoot, dataStorageLessonId, userId).isEmpty()) {
            ...
            String markLevelCompleteTest =
                Setter.updatePlayerResult(
                    applicationRoot, dataStorageLessonId, userId, "Feedback is Disabled", 1, 1, 1);
            if (markLevelCompleteTest != null) {
              String tournamentModules =
                  Getter.getTournamentModules(applicationRoot, userId, locale);
              if (!tournamentModules.isEmpty()) // Some Modules were included in response
              {
                // Get number of Challenges returned by getChallenges method
                int numberofChallengesReturned =
                    (tournamentModules.length()
                            - tournamentModules.replace("class='lesson'", "").length())
                        / "class='lesson'".length();
                if (numberofChallengesReturned > totalNumberOfModulesInShepherd) {
                  log.debug("Found " + numberofChallengesReturned + " modules");
                  if (!tournamentModules.contains(
                      "Corporal")) // English String Expected to be in the
                  // response when submitted with the
                  // locale for this unit test
                  {
                    ...
                    fail("Data Storage Lesson was not marked as complete in Tournament Menu");
                          ...
                          "<img src='css/images/uncompleted.png'/><a class='lesson' id='"
                              + insecureDirectObjectReferenceLesson)
                      == -1) {
          ...
          if (!Setter.setModuleStatusOpen(applicationRoot, sessionManagement8)) {
            ...
            if (!tournamentModules.isEmpty()) // Some Modules were included in response
            {
              ...
              if (numberofChallengesReturned == 1) {
                ...
                if (!tournamentModules.contains("Admiral")) // English String Expected to be in the
                // response when submitted with the locale
                // for this unit test
                {
  ...
  public void testIsCsrfLevelCompleteIncrementedCounter() {
    ...
    String csrfChallengeOne =
        new String("20e755179a5840be5503d42bb3711716235005ea"); // CSRF Challenge 1 (Should
  ...
  public void testIsCsrfLevelCompleteWithoutIncrementedCounter() {
    ...
    String csrfChallengeTwo =
        new String("94cd2de560d89ef59fc450ecc647ff4d4a55c15d"); // CSRF Challenge 2 (Should
    // have CSRF Counter of 0
    // for new user
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (!Getter.getModuleAddress(applicationRoot, csrfChallengeTwo, userId).isEmpty()) {
            if (!Getter.isCsrfLevelComplete(applicationRoot, csrfChallengeTwo, userId)) {
              ...
              // counter
              // of 0 and should not return true.
            }
          } else {
