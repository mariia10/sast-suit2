...
/**
 * Class is responsible for finding valid XSS and CSRF attacks in user submissions <br>
 * <br>
...
public class FindXSS {
   ...
   * vectors are all including in this detection method.
   *
  ...
  public static String[] colons = {":", "&#x3a", "&#x3a;", "&#58", "&#58;"};
          ...
          try {
            ...
            validUrl =
                csrfUrl
                        .getQuery()
                        .toLowerCase()
                        .equalsIgnoreCase(("userId=" + falseId).toLowerCase())
                    && validUrl;
            if (!validUrl) {
              log.debug("2");
            }
  ...
  /**
   * Searches for URL that contains CSRF attack string without user ID expected. Returns true if it
   * is valid based on parameters submitted
   *
  ...
  /**
   * Searches for URL that contains CSRF attack string. Returns true if it is valid based on
   ...
   * @return boolean value depicting if the attack is valid or not
   */
  public static boolean findCsrfAttackUrl(
      String theUrl,
      String csrfAttackPath,
      ...
      if (!validPath) {
        ...
        boolean validQuery =
            theAttack
                .getQuery()
                .toLowerCase()
                .equalsIgnoreCase((userIdParameterName + "=" + userIdParameterValue).toLowerCase());
        if (!validQuery) {
          log.debug(
              "Invalid Solution: Bad Query. Expected: "
                  + (userIdParameterName + "=" + userIdParameterValue).toLowerCase());
        } else {
          validAttack = true;
        }
      }
  ...
  public static boolean search(String xssString) {
    ...
    if (!xssDetected) // If Stage One failed, Move onto Stage 2 and 3
    {
      ...
      for (Element element : elements) {
        ...
        // Don't really care if they're in the correct element, the vector would have worked if they
        // had it in the right one.
        ...
        for (int i = 0; i < uriAttributes.length && !xssDetected; i++) {
          ...
          if (!uriAttributeValue.isEmpty()) {
            ...
            // URI Attacks need a Colon after data or javascript - Need to find that else it is
            // invalid
            ...
            for (int colonCount = 0; colonCount < colons.length; colonCount++) {
              ...
              if (colonFound) {
                ...
                uriAttributeValue =
                    uriAttributeValue.substring(0, uriAttributeValue.indexOf(colons[colonCount]));
